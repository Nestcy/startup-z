...
from app.models.outputs import SwitchingTriggersOutput
from app.services.search import SearchService
from app.services.model import ModelClient

PROMPT = """Given idea: {context[idea]}.
Evidence: {evidence}
Identify switching triggers (technology/regulation/market/behavior/economic).
Return JSON:
{"trigger_list": ["..."], "explanation":"...", "evidence":[{"text":"...", "url":"..."}], "confidence":0.0-1.0}
"""
...
class SwitchingTriggersNode(BaseNode):
    name = "switching_triggers"
    prompt_template = PROMPT
    reflection_prompt_template = REFLECTION_PROMPT
    output_model = SwitchingTriggersOutput
    ...
    def _make_search_query(self, context: Dict[str, Any]) -> str:
        return f"{context['idea']} trends drivers why users might switch 'why' 'adoption'"

    def _save_to_state(self, state: Any, validated: Any, evidence: Any, reflection: Any) -> None:
        state.switching_triggers = validated.dict()
